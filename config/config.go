...
func AddMount(backup *Backup, container *Service, source string, destination string) *Mount {
	...
	for _, mount := range backup.Mounts {
		if mount.MountSource == source && mount.MountDestination == mount.Service && mount.Service == container.Name {
			found = true
			new_mount = mount
		}
	}
	if !found {
		new_mount := Mount{MountSource: source, MountDestination: destination, Service: container.Name}
		backup.Mounts = append(backup.Mounts, &new_mount)
	}
	return new_mount
}
...
func AddSchema2PgDump(pgdump *PgDump, new_schema string) {
	found := false
	for _, schema := range pgdump.Schemas {
	...
	if !found {
		pgdump.Schemas = append(pgdump.Schemas, new_schema)
	}
}
